...
private:
	...
	{
		// generate triangle from 3 vertices using gs and send to clipper
		Triangle<GSOut> e = effect.gs(v0, v1, v2, triangle_index);
		ClipCullTriangle(e);
	}

	void ClipCullTriangle(Triangle<GSOut>& t)	// GSOut = VSOutput , be4 perspective division to NDC space
	{
		// cull triangle check if any vertices outside 
		...
		// clipping routines
		const auto Clip1 = [this](GSOut& v0, GSOut& v1, GSOut& v2)	// 1 vertex wrong side (v0)
			{
				...
				const GSOut v0a = interpolate(v0, v1, alphaA);
				const GSOut v0b = interpolate(v0, v2, alphaB);
				// draw triangles
				{
		...
		const auto Clip2 = [this](GSOut& v0, GSOut& v1, GSOut& v2)	// v0 , v1 wrong side
			{
				// calculate alpha values for getting adjusted vertices
	...
	{
		// perspective divide and transform all 3 vertices to screen space
	...
	// === triangle rasterization functions ===	Screen space
	...
	void DrawTriangle(const Triangle<GSOut>& triangle)
	{
		...
		else // general triangle
		{
			...
			const float alphaSplit =
				(pv1->pos.y - pv0->pos.y) /
				(pv2->pos.y - pv0->pos.y);
			const GSOut vi = interpolate(*pv0, *pv2, alphaSplit);
	...
	void DrawFlatTopTriangle(const GSOut& it0, const GSOut& it1, const GSOut& it2)
	{
		...
		const GSOut dit0 = (it2 - it0) / delta_y;
		const GSOut dit1 = (it2 - it1) / delta_y;
	...
	void DrawFlatBottomTriangle(const GSOut& it0, const GSOut& it1, const GSOut& it2)
	{
		...
		// change in interpolant for every 1 change in y
		const float delta_y = it2.pos.y - it0.pos.y;
		const GSOut dit0 = (it1 - it0) / delta_y;
		const GSOut dit1 = (it2 - it0) / delta_y;
					      ...
					      const GSOut& dv0, const GSOut& dv1, GSOut itEdge1)
	{
		...
		for (int y = yStart; y < yEnd; y++, itEdge0 += dv0, itEdge1 += dv1)	// x , y pixel , the others point vector
		{
			...
			// create scanline interpolant start point
			GSOut iLine = itEdge0;
			...
			for (int x = xStart; x < xEnd; x++, iLine += diLine)
			{
				...
				{
					// recover interpolated z from interpolated 1/z ( original z )
					const float w = 1.0f / iLine.pos.w;
					// recover interpolated attributes
					const GSOut attr = iLine * w;
