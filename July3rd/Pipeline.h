...
private:
	...
	void ProcessTriangle(const VSOut& v0, const VSOut& v1, const VSOut& v2, size_t triangle_index)
	{
		...
		// and send to clipper
		auto tris = effect.gs(v0, v1, v2, triangle_index);
		ClipCullTriangle(tris);
	}
		...
		const auto Clip1 = [this](GSOut& v0, GSOut& v1, GSOut& v2)
			{
				...
				// draw triangles
				auto tris1 = Triangle<GSOut>{ v0a, v1, v2 };
				PostProcessTriangleVertices(tris1);
				auto tris2 = Triangle<GSOut>{ v0b, v0a, v2 };
				PostProcessTriangleVertices(tris2);
			};
		const auto Clip2 = [this](GSOut& v0, GSOut& v1, GSOut& v2)
			{
				...
				// draw triangles
				auto tris = Triangle<GSOut>{ v0, v1, v2 };
				PostProcessTriangleVertices(tris);
			};
